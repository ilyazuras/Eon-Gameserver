...
void Looting::SetupLDSForPackage(TArray<FFortItemEntry>& LootDrops, SDK::FName Package, int i, FName TierGroup, int WorldLevel)
{
        ...
        if (LootDrop.ItemDefinition == ItemDefinition)
        {
            ...
            if (LootDrop.Count > (int32)Utils::EvaluateScalableFloat(ItemDefinition->MaxStackSize))
            {
                ...
                if (Inventory::GetQuickbar(LootDrop.ItemDefinition) == EFortQuickBars::Secondary)
                    LootDrops.Add(*Inventory::MakeItemEntry(ItemDefinition, OGCount - (int32)Utils::EvaluateScalableFloat(ItemDefinition->MaxStackSize),
    ...
    if (!found && LootPackage->Count > 0)
        LootDrops.Add(*Inventory::MakeItemEntry(ItemDefinition, LootPackage->Count,
            std::clamp(Inventory::GetLevel(ItemDefinition->LootLevelData), ItemDefinition->MinLevel, ItemDefinition->MaxLevel)));
}
...
TArray<FFortItemEntry> Looting::ChooseLootForContainer(FName TierGroup, int LootTier, int WorldLevel)
{
    ...
    for (auto& Item : LootDrops)
        if (Item.ItemDefinition->IsA<UFortWeaponRangedItemDefinition>() && !Item.ItemDefinition->IsStackable() &&
            ((UFortWorldItemDefinition*)Item.ItemDefinition)->GetAmmoWorldItemDefinition_BP())
        {
...
AFortPickup* Looting::SpawnPickup(UObject* Object, FFrame& Stack, AFortPickup** Ret)
{
    ...
    return *Ret = Inventory::SpawnPickup(Position, ItemDefinition, NumberToSpawn,
        ItemDefinition->IsA<UFortWeaponItemDefinition>() ? Inventory::GetStats((UFortWeaponItemDefinition*)ItemDefinition)->ClipSize : 0,
...
AFortPickup* Looting::K2_SpawnPickupInWorld(UObject* Object, FFrame& Stack, AFortPickup** Ret)
{
    ...
    return *Ret = Inventory::SpawnPickup(Position, ItemDefinition, NumberToSpawn, 0, SourceType, Source,
...
AFortPickup* Looting::SpawnItemVariantPickupInWorld(UObject* Object, FFrame& Stack, AFortPickup** Ret)
{
    ...
    return *Ret = Inventory::SpawnPickup(Params.Position, Params.WorldItemDefinition, Params.NumberToSpawn, 0,
...
void Looting::SpawnConsumableActor(ABGAConsumableSpawner* Spawner)
{
    ...
    auto GroundLoc = UFortKismetLibrary::FindGroundLocationAt(UWorld::GetWorld(), nullptr, Spawner->K2_GetActorLocation(), -1000, 2500, FName(L"FindGroundLocationAt"));
